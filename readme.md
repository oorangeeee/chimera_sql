...
- **自动化执行与报告**: 自动批量执行 SQL 并输出结构化报告，便于定位崩溃与语法兼容问题。
...
一条命令完成完整流水线：读取种子 SQL → AST 变异 → 方言转译 → 多数据库执行 → 生成报告。
...
| `--targets` | 否 | 逗号分隔的目标名称（默认 config.yaml 中所有 targets） |
...
```bash
# 对所有 config targets 运行（连不上的目标自动跳过）
python main.py run data/seeds -s sqlite

# 仅对 sqlite 运行
python main.py run data/seeds -s sqlite --targets sqlite_local

# 指定变异数量和随机种子
python main.py run data/seeds -s sqlite --targets sqlite_local -n 2 --seed 42
```
...
│   │   ├── runner.py              # CampaignRunner 流水线编排器
